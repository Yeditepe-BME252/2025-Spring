...
*** ($\S 3.3.1$)  Resultant Forces (Integrals)
#+ATTR_REVEAL: :frag (appear)
...
- and the resultant torque about a point $\boldsymbol{z}$ can be written as  $$\boldsymbol{\tau}_b[\Omega] = \int_\Omega (\boldsymbol{x} - \boldsymbol{z}) \times \rho(\boldsymbol{x}) \boldsymbol{b}(\boldsymbol{x}) dV.$$
...
- When the surface $\Gamma$ is part of of the bounding surface $\partial\mathcal{B}$, the external tractions imposed upon the body are denoted as $\bar{\boldsymbol{t}} = \boldsymbol{t} (\bar{\boldsymbol{n}}, \boldsymbol{x})$, where $\bar{\boldsymbol{n}}$ denotes the outward unit normal field.
  
** Resultant Forces (Integrals)
#+ATTR_HTML: :width 1200 px
...
+ and the resultant torque about point $\boldsymbol{z}$, $\boldsymbol{\tau}_s[\partial\Omega,\boldsymbol{z}]$ as   $$\boldsymbol{\tau}_s[\partial\Omega,\boldsymbol{z}] = \int_{\partial\Omega} (\boldsymbol{x} - \boldsymbol{z}) \times \bar{\boldsymbol{t}} dA.$$

...
- For this state of stress, we find the tractions on any surface with normal $\boldsymbol{n}$ at $\boldsymbol{x}$ is given by
  $$\boldsymbol{t} = \boldsymbol{\sigma} \boldsymbol{n} = -p \boldsymbol{n} \quad \mathrm{or} \quad t_i = -p \,n_i.$$
...
- For a uniaxial stress state the traction on any surface with normal $\boldsymbol{n}$ at $\boldsymbol{x}$ is given by
  $$\boldsymbol{t} = \boldsymbol{\sigma} \boldsymbol{n} = (\boldsymbol{\gamma} \cdot \boldsymbol{n}) \sigma \boldsymbol{\gamma} \quad \mathrm{or} \quad t_i = (\gamma_j n_j) \sigma \gamma_i$$
...
- For this stress state the traction on any surface with normal $\boldsymbol{n}$ is given by
  $$\boldsymbol{t} = \boldsymbol{\sigma} \boldsymbol{n} = (\boldsymbol{\eta} \cdot \boldsymbol{n}) \tau \boldsymbol{\gamma} + (\boldsymbol{\gamma} \cdot \boldsymbol{n}) \tau \boldsymbol{\eta} \quad \mathrm{or} \quad t_i = (\eta_j n_j) \tau \gamma_i + (\gamma_j n_j) \tau \eta_i.$$
...
[[../images/AFC-3.5.png]]
#+ATTR_REVEAL: :frag (appear)
- Let $\Omega$ be any open subset of $B$ and let $\boldsymbol{t}:\partial\Omega \to \mathcal{V}$ be the traction field acting on it's bounding surface.
- The resultant force on $\Omega$ due to body and surface forces is
  $$ \boldsymbol{r}[\Omega] = \boldsymbol{r}_b[\Omega] + \boldsymbol{r}_s[\partial\Omega] = \int_\Omega \boldsymbol{b}\, \rho\,dV + \int_{\partial\Omega} \bar{\boldsymbol{t}} \, dA $$
- The resultant torque on $\Omega$, about point $\boldsymbol{z}$, due to body and surface forces is
  $$ \boldsymbol{\tau}[\Omega,\boldsymbol{z}] = \boldsymbol{\tau}_b[\Omega,\boldsymbol{z}] + \boldsymbol{\tau}_s[\partial\Omega,\boldsymbol{z}] = \int_\Omega (\boldsymbol{x} - \boldsymbol{z}) \times \boldsymbol{b}\, \rho\,dV+ \int_{\partial\Omega} (\boldsymbol{x} - \boldsymbol{z}) \times  \bar{\boldsymbol{t}}\, dA $$
...
** Questions for Next Session
Ask AI:
- Briefly describe kinematics in the context of continuum body mechanics, to an engineering undergraduate.
